...
def cal_jacobian_63(theta3,
    ...
    jacobian_63 = np.array([[-d6*np.cos(theta3)*np.sin(theta2), -d6*np.sin(theta3)*np.cos(theta2), -np.sin(theta3)*np.sin(theta2)],
                            [0,                                 d6*np.sin(theta2),                 -np.cos(theta2)],
                            [d6*np.sin(theta3)*np.sin(theta2),  -d6*np.cos(theta3)*np.cos(theta2), -np.cos(theta3)*np.sin(theta2)]])
...
def cal_speed_control(x_dot, y_dot, z_dot, jacobian_matrix) -> np.array:
    ...
    jacobian_matrix: jacobian matrix of current status
    return: velocity of corresponding joints 3*1
    unit: mm/s
    '''
